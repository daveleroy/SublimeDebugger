...
class DebuggerState:
	...
	def launch(self, adapter_configuration: AdapterConfiguration, configuration: Configuration, breakpoints: Breakpoints) -> core.awaitable[None]:
		...
		adapter.onSelectedStackFrame.add(self._on_selected_frame)
		adapter.onScopes.add(self._on_scopes_event)
	...
	def stop(self) -> core.awaitable[None]:
		...
		if self.state == DebuggerState.stopping:
			self.force_stop_adapter()
			return
	...
	def resume(self) -> core.awaitable[None]:
		if not self.adapter:
			return
		yield from self.adapter.Resume()
	def pause(self) -> core.awaitable[None]:
		if not self.adapter:
			return
		yield from self.adapter.Pause()
	def step_over(self) -> core.awaitable[None]:
		if not self.adapter:
			return
		yield from self.adapter.StepOver()
	def step_in(self) -> core.awaitable[None]:
		if not self.adapter:
			return
		yield from self.adapter.StepIn()
	def step_out(self) -> core.awaitable[None]:
		if not self.adapter:
			return
		yield from self.adapter.StepOut()
	
	def _on_selected_frame(self, frame: Optional[StackFrame]) -> None:
		self.on_scopes([])
	...
	def _on_continued_event(self, event: Any) -> None:
		self.state = DebuggerState.running
