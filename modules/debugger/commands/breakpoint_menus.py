...
def edit_line_breakpoint(breakpoints: Breakpoints, breakpoint: Breakpoint, selected_index = 0):
	...
	def input(selected_index):
		return ui.ListInput(values, placeholder="Edit breakpoint @ line {}".format(breakpoint.line), index=selected_index)

	def on_cancel():
		core.run(open_file_and_cancel_highlight(window, breakpoint.file, breakpoint.line-1))
...
def edit_function_breakpoint(breakpoints: Breakpoints, breakpoint: FunctionBreakpoint, selected_index = 0):
	...
	def input(selected_index):
		return ui.ListInput(values, placeholder="Edit breakpoint @ function {}".format(breakpoint.name), index=selected_index)
