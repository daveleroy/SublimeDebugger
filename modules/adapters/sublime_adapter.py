...
class SublimeInstaller(dap.AdapterInstaller):
	...
	async def install(self, version: str | None, log: core.Logger) -> None:
		...
		core.write(f'{path}/package.json', core.json_encode({'version': version}))
...
class Sublime(dap.Adapter):
	...
	async def start(self, console: dap.Console, configuration: dap.ConfigurationExpanded):
		...
		return SublimeDebugTransport(configuration, console)
...
class SublimeDebugTransport(dap.Transport):
	def __init__(self, configuration: dap.ConfigurationExpanded, log: core.Logger) -> None:
		...
		env = {'sublime_debug_configuration': core.json_encode(configuration)}
