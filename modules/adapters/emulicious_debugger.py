...
class EmuliciousDebugger(dap.Adapter):
	...
	installer = util.GitInstaller(type='emulicious-debugger', repo='calindro/emulicious-debugger')
	...
	async def start(self, console: dap.Console, configuration: dap.ConfigurationExpanded):
		...
		if check_port_socket.connect_ex((configuration.get('host') or 'localhost', configuration.get('port'))) != 0:
			return await self.startEmulicious(console, configuration)
	...
	async def startEmulicious(self, console: dap.Console, configuration: dap.ConfigurationExpanded):
		...
		cwd = await configuration.variables['folder']
		startupinfo = None
		if os.name == 'nt':
